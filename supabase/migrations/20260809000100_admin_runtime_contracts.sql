...
-- 7. Repair teacher assessment RPCs that must match the actual Phase 6 schema.
...
REVOKE EXECUTE ON FUNCTION public.create_exam_assignment(
  TEXT, UUID, UUID, INT, NUMERIC, NUMERIC, TEXT
) FROM PUBLIC;
...
-- 8. Refresh PostgREST schema cache so newly-created RPCs are visible immediately.
